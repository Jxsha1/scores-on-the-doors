...
async function fetchFixtures() {
    ...
    elements.fixtures.innerHTML = data?.map(f => `
        <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-4" data-id="${f.fixture_id}">
            <div class="flex justify-between text-[10px] font-bold text-gray-400 uppercase mb-2">
                ...
                <span>${f.status === 'finished' ? 'Final Score' : 'Upcoming'}</span>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex-1 text-right font-bold">${f.home_team}</div>
                ${f.status === 'finished' ? 
                    `<div class="px-4 font-black text-xl">${f.home_score_actual} - ${f.away_score_actual}</div>` :
                    `<div class="flex gap-1 mx-2">
                        <input type="number" id="h-${f.fixture_id}" class="w-10 h-10 text-center border-2 border-gray-100 rounded-lg font-bold" placeholder="-">
                        <input type="number" id="a-${f.fixture_id}" class="w-10 h-10 text-center border-2 border-gray-100 rounded-lg font-bold" placeholder="-">
                    </div>`
                }
...
// --- SCORE UPDATING (The Secret Sauce) ---
window.updateMatchResult = async (id) => {
    ...
    // Call the Postgres function we created in Step 1
    const { error } = await sbClient.rpc('calculate_fixture_points', {
        target_fixture_id: id,
        final_home: h,
        final_away: a
    });
    ...
    if (error) alert("Admin Error: " + error.message);
        ...
        alert("Match Updated & Points Calculated!");
        fetchAdminFixtures();
    }
};
...
async function fetchLeaderboard() {
    ...
    elements.leaderboard.innerHTML = data?.map((u, i) => `
        <tr class="border-b border-gray-50">
            <td class="px-4 py-4 text-gray-400 font-bold">#${i+1}</td>
            <td class="px-4 py-4 font-semibold">${u.display_name.split('@')[0]}</td>
            <td class="px-4 py-4 text-right font-black text-blue-900">${u.total_points}</td>
        </tr>
    `).join('') || '';
}
...
elements.submitBtn.onclick = async () => {
    ...
    document.querySelectorAll('[data-id]').forEach(div => {
        ...
        if (h && a) preds.push({ uid: currentUser.id, fixture_id: id, home_predicted: h, away_predicted: a });
    });
    const { error } = await sbClient.from('predictions').upsert(preds, { onConflict: 'uid, fixture_id' });
    alert(error ? error.message : "Predictions Locked In!");
};
...
sbClient.auth.onAuthStateChange((_, session) => {
    ...
    elements.loginBtn.className = currentUser ? "bg-red-600 px-4 py-2 rounded text-sm font-semibold" : "bg-blue-600 px-4 py-2 rounded text-sm font-semibold text-white";
    ...
    elements.status.textContent = currentUser ? `Logged in: ${currentUser.email}` : "Sign in to predict.";
    fetchFixtures();
});
...
elements.authForm.onsubmit = async (e) => {
    ...
    const { error } = isSignUpMode ? await sbClient.auth.signUp({ email, password: pass }) : await sbClient.auth.signInWithPassword({ email, password: pass });
